...
<script language="javascript">
	...
	window.onload=function(){
		...
		document.getElementById("nextStep").onclick=function(){
			window.open("paySystem?payment="+total,"_self");		
		};	
		document.getElementById("clearCart").onclick=function(){
			window.open("delCart","_self");
	...
	{
		window.open("modifyCart?add=true&&id="+id,"_self");	
	}
	function minusfunc(id)
	{
		window.open("modifyCart?minus=true&&id="+id,"_self");	
	}
		...
		<c:forEach items="${sessionScope.cartList}" var="i">
			<tr>
				...
				<td width="40%" align="center">
					...
					<a href="delCartItem?id=${i.id}">删除</a> &nbsp &nbsp 
					<a href="detail?id=${i.id}">查看详情</a>
				</td>
			</tr>
			</c:forEach>
			<tr>
